    ...
    <div class="stage">
          ...
          <div class="ov-right-col">
            ...
            <div class="ov-cards-row ov-row-bottom">
              ...
              <div class="ov-card ov-card-community" style="--d:0.32s">
                ...
                <div class="ov-orbit">
                  <div class="ov-orbit-ring ov-orbit-1">
                    <div class="ov-orbit-dot"></div>
                  </div>
                  <div class="ov-orbit-ring ov-orbit-2">
                    <div class="ov-orbit-dot ov-orbit-dot-2"></div>
                  </div>
                </div>
              </div>
      ...
      <div v-if="isSlideEnabled(1)" class="slide" :class="{sin: cur===1, sout: exitSlide===1}">
        <div class="slide-inner sentiment-slide">
          <div class="ss-scene" :class="'ss-phase-'+iosPhase">
            <div class="ss-wm-text">{{ currentIosSentiment.label.toUpperCase() }}</div>
            <div class="ss-face-stage" :class="'sf-'+currentIosSentiment.key">
              <div class="sf-ring sf-ring-1"></div>
              <div class="sf-ring sf-ring-2"></div>
              <div class="sf-glow"></div>
              <div class="sf-ring sf-ring-3"></div>
              <EmotionFace :key="'ios-face-'+iosSentimentIdx" :emotionKey="currentIosSentiment.key" class="ss-hero-face" />
            ...
            <div class="ss-left-panel" :class="'sf-'+currentIosSentiment.key">
              <div class="slp-glow"></div>
              <div class="slp-title">WHAT THEY<br><span class="slp-accent">SAID<br> <small class="slp-as">AS</small></span></div>
              <EmotionFace :key="'ios-sm-'+iosSentimentIdx" :emotionKey="currentIosSentiment.key" class="ss-panel-face" />
              <div class="slp-badge">
                <div class="slp-sent-name">{{ currentIosSentiment.label }}</div>
                <div class="slp-rev-count">{{ iosSentimentGroups[currentIosSentiment.key]?.length || 0 }} reviews</div>
              </div>
              <div class="ss-progress-dots">
                <span v-for="(s,i) in sentimentOrder" :key="s.key" class="ss-sdot" :class="i===iosSentimentIdx ? 'sdot-on' : ''"></span>
              </div>
            </div>
            <div class="ss-carousel-panel">
              <div class="carousel-viewport">
                <div class="carousel-track">
                  <div
                    v-for="(rev, ridx) in (iosSentimentGroups[currentIosSentiment.key] || [])"
                    :key="iosSentimentIdx + '-' + ridx"
                    class="carousel-card"
                    :class="getCarouselClass(ridx, iosRevIdxInGroup, iosSentimentGroups[currentIosSentiment.key]?.length || 0)"
                    :style="getCarouselStyle(ridx, iosRevIdxInGroup, iosSentimentGroups[currentIosSentiment.key]?.length || 0, sentimentColor(currentIosSentiment.key))"
                  >
                    <div class="cc-deco-blob" :style="{background: sentimentColor(currentIosSentiment.key)}"></div>
                    <div class="cc-author-row">
                      <div class="cc-avatar" :style="{background: sentimentColor(currentIosSentiment.key)+'22', color: sentimentColor(currentIosSentiment.key)}">{{ (rev.author||'A')[0].toUpperCase() }}</div>
                      <div class="cc-meta">
                        <div class="cc-author">{{ rev.author || 'Anonymous' }}</div>
                        <div class="cc-stars">
                          <span v-for="si in 5" :key="si" class="cc-star" :class="getStarClass(rev.rating||0, si)"></span>
                          <span class="cc-date">· {{ rev.date || 'Recently' }}</span>
                        </div>
                      </div>
                    </div>
                    <div class="cc-quote">
                      <span class="cc-qmark" :style="{color: sentimentColor(currentIosSentiment.key)}">"</span>
                      <div class="cc-body">{{ rev.body }}</div>
                    </div>
                    <div class="cc-footer">
                      <div class="cc-timing-bar">
                        <div class="cc-timing-fill" :style="{width: iosRevProgressPct+'%', background: sentimentColor(currentIosSentiment.key)}"></div>
                      </div>
                    </div>
                  </div>
                </div>
          ...
          <div class="ss-group-tabs">
            <div v-for="(s,i) in sentimentOrder" :key="s.key"
               v-show="(iosSentimentGroups[s.key] || []).length > 0"
               class="ss-group-tab"
               :class="[('tab-'+s.key), i===iosSentimentIdx ? 'tab-on' : '']"
               @click="jumpIosSentiment(i)">
              <span class="ss-tab-inner">{{ s.emoji }}</span>
            </div>
          </div>
        </div>
        <canvas v-show="cur===1 && currentIosSentiment.key==='ecstatic'" ref="iosCeleCanvas" class="celebration-canvas"></canvas>
        <canvas v-show="cur===1 && (currentIosSentiment.key==='frustrated' || currentIosSentiment.key==='angry')" ref="iosCommCanvas" class="commiseration-canvas"></canvas>
      </div>

      <!-- ══════════════ SLIDE 2: PLAY STORE ══════════════ -->
      <div v-if="isSlideEnabled(2)" class="slide" :class="{sin: cur===2, sout: exitSlide===2}">
        <div class="slide-inner sentiment-slide">
          <div class="ss-scene" :class="'ss-phase-'+andPhase">
            <div class="ss-wm-text">{{ currentAndSentiment.label.toUpperCase() }}</div>
            <div class="ss-face-stage" :class="'sf-'+currentAndSentiment.key">
              <div class="sf-ring sf-ring-1"></div>
              <div class="sf-ring sf-ring-2"></div>
              <div class="sf-ring sf-ring-3"></div>
              <div class="sf-glow"></div>
              <EmotionFace :key="'and-face-'+andSentimentIdx" :emotionKey="currentAndSentiment.key" class="ss-hero-face" />
              <div class="sf-label-wrap">
            ...
            <div class="ss-left-panel" :class="'sf-'+currentAndSentiment.key">
              <div class="slp-glow"></div>
              <div class="slp-title">WHAT THEY<br><span class="slp-accent">SAID<br> <small class="slp-as">AS</small></span></div>
              <EmotionFace :key="'and-sm-'+andSentimentIdx" :emotionKey="currentAndSentiment.key" class="ss-panel-face" />
              <div class="slp-badge">
                <div class="slp-sent-name">{{ currentAndSentiment.label }}</div>
                <div class="slp-rev-count">{{ andSentimentGroups[currentAndSentiment.key]?.length || 0 }} reviews</div>
              </div>
              <div class="ss-progress-dots">
            ...
            <div class="ss-carousel-panel">
              <div class="carousel-viewport">
                <div class="carousel-track">
                  <div
                    v-for="(rev, ridx) in (andSentimentGroups[currentAndSentiment.key] || [])"
                    :key="andSentimentIdx + '-' + ridx"
                    class="carousel-card"
                    :class="getCarouselClass(ridx, andRevIdxInGroup, andSentimentGroups[currentAndSentiment.key]?.length || 0)"
                    :style="getCarouselStyle(ridx, andRevIdxInGroup, andSentimentGroups[currentAndSentiment.key]?.length || 0, sentimentColor(currentAndSentiment.key))"
                  >
                    <div class="cc-deco-blob" :style="{background: sentimentColor(currentAndSentiment.key)}"></div>
                    <div class="cc-author-row">
                      <div class="cc-avatar" :style="{background: sentimentColor(currentAndSentiment.key)+'22', color: sentimentColor(currentAndSentiment.key)}">{{ (rev.author||'A')[0].toUpperCase() }}</div>
                      <div class="cc-meta">
                        <div class="cc-author">{{ rev.author || 'Anonymous' }}</div>
                        <div class="cc-stars">
                          <span v-for="si in 5" :key="si" class="cc-star" :class="getStarClass(rev.rating||0, si)"></span>
                    ...
                    <div class="cc-quote">
                      <span class="cc-qmark" :style="{color: sentimentColor(currentAndSentiment.key)}">"</span>
                      <div class="cc-body">{{ rev.body }}</div>
                    </div>
                    <div class="cc-footer">
                      <div class="cc-timing-bar">
                        <div class="cc-timing-fill" :style="{width: andRevProgressPct+'%', background: sentimentColor(currentAndSentiment.key)}"></div>
                      </div>
                    </div>
                  </div>
                </div>
          ...
          <div class="ss-group-tabs">
            <div v-for="(s,i) in sentimentOrder" :key="s.key"
               v-show="(andSentimentGroups[s.key] || []).length > 0"
               class="ss-group-tab"
        ...
        <canvas v-show="cur===2 && currentAndSentiment.key==='ecstatic'" ref="andCeleCanvas" class="celebration-canvas"></canvas>
        <canvas v-show="cur===2 && (currentAndSentiment.key==='frustrated' || currentAndSentiment.key==='angry')" ref="andCommCanvas" class="commiseration-canvas"></canvas>
      </div>
      ...
      <div v-if="isSlideEnabled(4)" class="slide" :class="{sin: cur===4, sout: exitSlide===4}">
        <div class="slide-inner xf-slide" :class="'xf-phase-'+xPhase">
          <div class="xf-bg-glow"></div>
          <div class="xf-wm">X FEED</div>
          <div class="xf-hero-stage">
            <div class="xf-hero-ring xf-ring-1"></div>
            <div class="xf-hero-ring xf-ring-2"></div>
            <transition name="xf-pop" mode="out-in">
              <div class="xf-hero-shot" :key="'hero-'+twitterIdx">
                <img v-if="(twitter.recent_posts||[])[twitterIdx]?.screenshot_url && !xImageFallback"
                  ...
                  class="xf-hero-img" @error="xImageFallback=true" alt="tweet"/>
                <div v-else class="xf-hero-text">{{ (twitter.recent_posts||[])[twitterIdx]?.body || 'Loading post…' }}</div>
              </div>
            </transition>
          </div>
            ...
            <transition name="xf-pop" mode="out-in">
              <div class="xf-left-shot" :key="'left-'+twitterIdx">
                ...
                <div v-else class="xf-left-text-block">
                  ...
                  <!-- Post images (pbs.twimg.com media) if screenshot is unavailable -->
          ...
          <div class="xf-right-panel">
            <div class="xf-right-header">
              <span class="xf-right-eyebrow">TOP REPLIES</span>
              <span class="xf-right-count">{{ ((twitter.recent_posts||[])[twitterIdx]?.comments||[]).length }} comments</span>
            </div>
            <div class="xf-comments-viewport" :key="'cv-'+twitterIdx">
              <div class="xf-comments-track">
                <div v-for="(comment, ci) in ((twitter.recent_posts||[])[twitterIdx]?.comments||[]).slice(0,5)"
                  :key="twitterIdx+'-c'+ci"
                  class="xf-comment-card"
                  :class="getCommentClass(ci, xCommentIdx, Math.min(((twitter.recent_posts||[])[twitterIdx]?.comments||[]).length,5))"
                  :style="getCommentStyle(ci, xCommentIdx, Math.min(((twitter.recent_posts||[])[twitterIdx]?.comments||[]).length,5))">
                  <div class="xfc-top">
                    <div class="xfc-avatar">{{ (comment.author||'U')[0].toUpperCase() }}</div>
                    <div class="xfc-meta"><div class="xfc-author">{{ comment.author }}</div></div>
                  </div>
                  <div class="xfc-text">{{ comment.body }}</div>
                </div>
              </div>
              <div v-if="!((twitter.recent_posts||[])[twitterIdx]?.comments?.length)" class="xf-no-comments">No replies for this post</div>
            </div>
            <div class="xf-comment-dots">
              <span v-for="ci in Math.min(((twitter.recent_posts||[])[twitterIdx]?.comments||[]).length,5)" :key="ci"
                class="xf-dot" :class="{active: (ci-1)===xCommentIdx}"></span>
            </div>
          </div>
        </div>
      </div>
...
function scheduleNextTwitter(){
  ...
  const comments=((posts[twitterIdx.value]?.comments)||[]).slice(0,5)
  const currentComment=comments[xCommentIdx.value]
  const delay=currentComment?.timing ? currentComment.timing * 1000 : 3500

  if(comments.length>1&&xCommentIdx.value<comments.length-1){commentLoopTimeout=setTimeout(()=>{xCommentIdx.value=xCommentIdx.value+1;scheduleNextTwitter()},delay)}
  else{twitterLoopTimeout=setTimeout(()=>{twitterIdx.value=(twitterIdx.value+1)%posts.length;xCommentIdx.value=0;triggerXPhase();scheduleNextTwitter()},delay)}
}
...
/* LOADING */
.loading-screen{position:fixed;inset:0;z-index:9999;background:var(--y-pale);display:flex;align-items:center;justify-content:center;overflow:hidden;}
.load-blob{position:absolute;border-radius:50%;filter:blur(80px);animation:ld-drift 5s ease-in-out infinite alternate;}
.load-blob-1{width:500px;height:500px;background:var(--y-main);opacity:.25;top:-160px;left:-160px;}
.load-blob-2{width:400px;height:400px;background:var(--y-deep);opacity:.2;bottom:-100px;right:-100px;animation-delay:-2.5s;}
.load-blob-3{width:260px;height:260px;background:var(--y-soft);opacity:.35;top:40%;left:42%;animation-delay:-1.2s;}
@keyframes ld-drift{to{transform:translate(60px,40px) scale(1.15);}}
.load-content{position:relative;z-index:1;text-align:center;display:flex;flex-direction:column;align-items:center;gap:10px;}
...
.cc-body{font-family:var(--font-b);font-size:clamp(15px,1.8vw,22px);line-height:1.7;color:var(--ink);font-weight:500;flex:1;overflow:hidden;display:-webkit-box;-webkit-box-orient:vertical;-webkit-line-clamp:10;align-self:center;text-align:center;}
...
/* X FEED SLIDE */
.xf-slide{flex-direction:row;align-items:stretch;padding:0;position:relative;overflow:hidden;}
.xf-bg-glow{position:absolute;width:700px;height:700px;top:50%;left:50%;transform:translate(-50%,-50%);border-radius:50%;background:radial-gradient(circle,rgba(29,161,242,.12) 0%,transparent 70%);filter:blur(60px);pointer-events:none;z-index:0;animation:xf-glow-pulse 4s ease-in-out infinite alternate;}
@keyframes xf-glow-pulse{from{opacity:.6;transform:translate(-50%,-50%) scale(1);}to{opacity:1;transform:translate(-50%,-50%) scale(1.15);}}
.xf-wm{position:absolute;inset:0;display:flex;align-items:center;justify-content:center;font-family:var(--font-d);font-size:clamp(100px,16vw,200px);font-weight:900;letter-spacing:-.04em;color:rgba(29,161,242,.06);pointer-events:none;user-select:none;z-index:0;transition:opacity .6s;}
.xf-phase-split .xf-wm{opacity:.03;}
.xf-hero-stage{position:absolute;inset:0;z-index:5;display:flex;align-items:center;justify-content:center;opacity:1;transform:scale(1);transition:opacity .6s cubic-bezier(.4,0,.2,1),transform .7s cubic-bezier(.16,1,.3,1);}
.xf-phase-split .xf-hero-stage{opacity:0;transform:scale(.8) translateX(-80px);pointer-events:none;}
.xf-hero-ring{position:absolute;border-radius:50%;border:1.5px solid rgba(29,161,242,.35);pointer-events:none;animation:xf-ring 3.5s ease-out infinite;}
.xf-ring-1{width:460px;height:460px;animation-delay:0s;}
.xf-ring-2{width:580px;height:580px;animation-delay:1.2s;}
@keyframes xf-ring{0%{opacity:.5;transform:scale(.7);}100%{opacity:0;transform:scale(1.2);}}
.xf-hero-shot{position:relative;z-index:2;max-width:420px;width:38vw;border-radius:20px;overflow:hidden;box-shadow:0 0 0 6px rgba(29,161,242,.12),0 32px 80px rgba(0,0,0,.22);animation:xf-hero-arrive .7s cubic-bezier(.34,1.56,.64,1) both;}
@keyframes xf-hero-arrive{from{opacity:0;transform:scale(.6) rotate(-4deg);}to{opacity:1;transform:scale(1) rotate(0deg);}}
.xf-pop-enter-active{transition:all .5s cubic-bezier(.22,1,.36,1);}
.xf-pop-leave-active{transition:all .25s ease;}
.xf-pop-enter-from{opacity:0;transform:scale(.85) translateY(20px);}
.xf-pop-leave-to{opacity:0;transform:scale(1.08);}
.xf-hero-img{width:100%;height:auto;display:block;object-fit:contain;}
.xf-hero-text{padding:2.5rem;font-family:var(--font-b);font-size:clamp(18px,2.2vw,28px);line-height:1.6;color:var(--ink);background:#fff;text-align:center;min-height:220px;display:flex;align-items:center;justify-content:center;}
.xf-left-panel{grid-column:1;width:44%;flex-shrink:0;display:flex;flex-direction:column;align-items:center;justify-content:flex-start;gap:.8rem;padding:1.2rem 1.5rem 1rem 2.5rem;position:relative;z-index:6;opacity:0;transform:translateX(-80px);transition:opacity .6s .2s cubic-bezier(.4,0,.2,1),transform .7s .2s cubic-bezier(.16,1,.3,1);pointer-events:none;}
.xf-phase-split .xf-left-panel{opacity:1;transform:translateX(0);pointer-events:all;}
...
.xf-left-shot{width:100%;max-width:460px;max-height:calc(100vh - 230px);border-radius:16px;overflow:hidden;background:#fff;display:flex;align-items:flex-start;justify-content:center;box-shadow:0 0 0 4px rgba(29,161,242,.1),0 16px 40px rgba(0,0,0,.15);}
.xf-left-img{width:100%;max-height:calc(100vh - 230px);height:100%;display:block;object-fit:contain;object-position:top center;}
...
.xf-post-img{width:100%;border-radius:10px;object-fit:cover;max-height:220px;border:1px solid rgba(29,161,242,.12);box-shadow:0 4px 14px rgba(0,0,0,.08);}
...
.xf-right-panel{flex:1;display:flex;flex-direction:column;gap:1rem;padding:2rem 2.5rem 2rem 2rem;position:relative;z-index:6;opacity:0;transform:translateX(80px);transition:opacity .6s .3s cubic-bezier(.4,0,.2,1),transform .7s .3s cubic-bezier(.16,1,.3,1);pointer-events:none;}
.xf-phase-split .xf-right-panel{opacity:1;transform:translateX(0);pointer-events:all;}
.xf-right-header{display:flex;align-items:baseline;gap:14px;padding-bottom:.8rem;border-bottom:2px solid rgba(29,161,242,.18);flex-shrink:0;}
...
.xf-comments-viewport{flex:1;min-height:0;position:relative;perspective:1200px;perspective-origin:50% 50%;overflow:visible;}
.xf-comments-track{position:absolute;inset:0;transform-style:preserve-3d;}
.xf-comment-card{position:absolute;top:50%;left:50%;width:min(92%,780px);transform:translate(-50%,-50%);border-radius:22px;background:#ffffff;border:1.5px solid rgba(29,161,242,.1);padding:2.2rem 2.8rem;display:flex;flex-direction:column;gap:1.4rem;overflow:hidden;box-shadow:0 8px 32px rgba(0,0,0,.07);transition:transform .65s cubic-bezier(.16,1,.3,1),opacity .5s cubic-bezier(.16,1,.3,1),filter .5s ease;will-change:transform,opacity;}
.xfc-active{transform:translate(-50%,-50%) translateZ(0) scale(1);opacity:1;filter:none;z-index:10;box-shadow:0 2px 0 rgba(255,255,255,.9) inset,0 24px 60px rgba(0,0,0,.13);border-color:rgba(29,161,242,.2);border-left:5px solid #1DA1F2;}
.xfc-next{transform:translate(-18%,-50%) translateZ(-200px) rotateY(-20deg) scale(0.84);opacity:0.5;filter:brightness(0.85);z-index:5;}
.xfc-prev{transform:translate(-82%,-50%) translateZ(-200px) rotateY(20deg) scale(0.84);opacity:0.5;filter:brightness(0.85);z-index:5;}
.xfc-far-next{transform:translate(2%,-50%) translateZ(-380px) rotateY(-32deg) scale(0.65);opacity:0.22;filter:brightness(0.72);z-index:2;}
.xfc-far-prev{transform:translate(-102%,-50%) translateZ(-380px) rotateY(32deg) scale(0.65);opacity:0.22;filter:brightness(0.72);z-index:2;}
.xfc-hidden{opacity:0;pointer-events:none;z-index:0;transform:translate(-50%,-50%) translateZ(-650px) scale(0.3);}
.xfc-top{display:flex;align-items:center;gap:16px;padding-bottom:1.2rem;border-bottom:1.5px solid rgba(29,161,242,.1);flex-shrink:0;}
.xfc-avatar{width:52px;height:52px;border-radius:50%;background:linear-gradient(135deg,#1DA1F2,#0A7BC5);color:#fff;display:flex;align-items:center;justify-content:center;font-family:var(--font-d);font-size:22px;font-weight:900;flex-shrink:0;box-shadow:0 4px 14px rgba(29,161,242,.3);}
.xfc-meta{display:flex;flex-direction:column;gap:4px;}
.xfc-author{font-family:var(--font-d);font-size:clamp(16px,1.8vw,22px);font-weight:800;color:var(--ink);line-height:1.2;}
.xfc-text{font-family:var(--font-b);font-size:clamp(18px,2.2vw,28px);line-height:1.65;color:var(--ink);font-weight:500;flex:1;overflow:hidden;display:-webkit-box;-webkit-box-orient:vertical;-webkit-line-clamp:6;}
.xf-comment-dots{display:flex;gap:8px;justify-content:center;padding-top:.5rem;flex-shrink:0;}
.xf-no-comments{position:absolute;inset:0;display:flex;align-items:center;justify-content:center;font-family:var(--font-b);font-size:18px;color:var(--ink3);font-style:italic;}
</style>
